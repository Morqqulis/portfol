...
const Navigation: React.FC = (): JSX.Element => {
    ...
    return (
        <nav className="flex justify-between items-center text-[#607B96] h-full">
                ...
                >
                    {navigationsSettings.map((navigationItem) => (
                        <li className="sm:last:hidden" key={navigationItem.id}>
                            <MyLink
                                className={
                                    `px-8 border mmd:px-5 hover:text-white duration-500 border-border-color block py-[17px] ${pathname === navigationItem.href ? 'border-b-primary-button text-white' : ''}`
                                }
            ...
            <MyLink
                className={`hidden sm:block px-8 border mmd:px-5 hover:text-white duration-500 border-border-color py-[17px] ${pathname === navigationsSettings[3].href ? 'border-b-primary-button text-white' : ''}`}
